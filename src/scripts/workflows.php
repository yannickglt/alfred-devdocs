...
class Workflows {
  ...
  public function fetch($url) {
    $httpProxy = isset($_ENV['HTTP_PROXY']) ? $_ENV['HTTP_PROXY'] : '';
    if (!empty($httpProxy)) {
      $context = array(
        'http' => array(
      ...
      $httpProxyAuth = isset($_ENV['HTTP_PROXY_AUTHORIZATION']) ? $_ENV['HTTP_PROXY_AUTHORIZATION'] : '';
      if (!empty($httpProxyAuth)) {
        $context['http']['header'] = "Proxy-Authorization: Basic $httpProxyAuth";
      }
